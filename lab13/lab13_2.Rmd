...
table_A <- read_csv("data/table_A.csv") %>% clean_names()
```
...
```{r}
table_B <- read_csv("data/table_B.csv") %>% clean_names()
```
```{r}
table_B
```
...
#strict way of joining, as it only keeps the rows that have matching values in both tables
```
...
#trouble shoot problematic joins
```
...
```{r}
joined_spider <- full_join(spider_records, spider_locations, by="accession")
```
...
```{r}
joined_spider %>%
  write_csv("data/joined_spider.csv")
```
