...
```{r}
ggmap(map1) +
  geom_point(data=spiders,aes(longitude,latitude),size=0.75,colar="black",alpha=0.8)+
...
```{r message=FALSE, warning=FALSE}
bears <- read_csv("data/bear-sightings.csv")%>% 
  clean_names()
```
...
```
```{r}
lat_bears <- c(55.02, 70.37)
long_bears <- c(-166.2, -131.3)
...
```{r}
map2 <- get_stadiamap(bbox_bears, maptype="stamen_terrain", zoom = 5)
```
...
```{r}
ggmap(map2) +
  geom_point(data=bears,aes(longitude,latitude),size=0.75,color="blue",alpha=0.8)+
  labs(x="Longitude",y="Latitude",title="Alaska Grizzly Bears Distribution")
```
