...
\
\The study is testing about wheter the illegal harvesting activity affected the tusk length, evolutionarily on the newborn elephants. Since the elder one with long tusk have being hunted and whether that will have an impact on the tusk length of newborn.
\
\
\
...
```{r}
elephants <-elephants %>% clean_names()
```
...
e_new %>%
  ...
  summarize(old_f_mean = mean(estimated_age_years,na.rm = T),
            old_f_median = median(estimated_age_years,na.rm = T),
            old_f_sd = sd(estimated_age_years,na.rm = T),
            )
e_new %>%
  filter(years_of_sample_collection == "1966-68" & sex == "m") %>%
  summarize(old_m_mean = mean(estimated_age_years,na.rm = T),
            old_m_median = median(estimated_age_years,na.rm = T),
            old_m_sd = sd(estimated_age_years,na.rm = T),
            )
e_new %>%
  filter(years_of_sample_collection == "2005-13" & sex == "f") %>%
  summarize(new_f_mean = mean(estimated_age_years,na.rm = T),
            new_f_median = median(estimated_age_years,na.rm = T),
            new_f_sd = sd(estimated_age_years,na.rm = T),
            )
e_new %>%
  filter(years_of_sample_collection == "2005-13" & sex == "m") %>%
  summarize(new_m_mean = mean(estimated_age_years,na.rm = T),
            new_m_median = median(estimated_age_years,na.rm = T),
            new_m_sd = sd(estimated_age_years,na.rm = T),
            )
```
\
\
...
```{r}
e_new %>%
  ggplot(aes(x=years_of_sample_collection,y=tusk_circumference_in_cm))+
  geom_boxplot()
```
