...
msleep %>%
  filter(bodywt>100) %>%
  summarize(n_genru = n_distinct(genus))
```
...
```{r}
msleep %>%
  summarize(total = n_distinct(genus))
```
...
msleep %>%
  ...
  geom_col() #use this when haveing an x and y
```
...
```{r}
getwd()
setwd("/Users/daledeng/Desktop/BIS15L_W26_RenxiaoDeng/lab7")
```
