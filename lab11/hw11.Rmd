...
```{r}
gapminder %>%
  filter(year %in% c(1925, 2025)) %>%     # keep only start & end years
  group_by(country) %>%
  summarise(
    pop_1925 = pop[year == 1925],
    pop_2025 = pop[year == 2025],
    abs_growth = pop_2025 - pop_1925
  ) %>%
  arrange(desc(abs_growth)) %>%           # largest growth first
  slice_head(n = 5)        
```
