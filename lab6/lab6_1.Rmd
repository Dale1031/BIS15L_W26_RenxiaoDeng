...
```{r}
library("tidyverse")
library("janitor") #this wll help with cleaning
...
```{r}
penguins %>%
  distinct(species,island,.keep_all = T) #keep all to select a guy in the unique, can select to select certain columns to be presented
```
...
```{r}
mammals %>%
  mutate(across(c("order","family"),tolower)) #To specific columns
```

This will change all columns to lower case. But, notice what happens to numeric columns.
```{r}
mammals %>%
  mutate(across(everything(),tolower)) #Changes numeric variable to character
```
...
mammals %>%
  ...
  mutate(newborn_new=if_else(newborn == -999.00, NA, newborn))
```
...
mammals %>% 
  ...
  mutate(max_life_y = max_life/12) %>%
  arrange(desc(max_life_y))
```
