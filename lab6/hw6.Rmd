...
```{r}
gabon_new <- gabon %>% mutate(across(c("hunt_cat","land_use","transect_id"),as.factor))
```
...
gabon_new %>%
  ...
  ggplot(aes(x=land_use,y=diversity_all_species))+
  geom_boxplot()+
  labs(y="speciese diversity")
```
...
```{r}
gabon_new %>%
  ggplot(aes(x=veg_rich,y=rich_mammal_species))+
  geom_point()+
...
```{r}
gabon_new %>% filter(veg_rich>15 | between(veg_dbh,50,60))
```
...
```{r}
gabon_new %>% filter(veg_rich > 10 & rich_mammal_species > 10) %>% arrange(veg_rich)
```
