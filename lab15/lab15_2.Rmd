...
ui <- fluidPage(
  ...
  selectInput("y",
              ...
              choices = c("bodymass",
                          ...
                          "gestationlength"),
              selected = "species"),
  helpText("The data are from: Thodoris Danis, Antonis Rokas; The evolution of gestation length in eutherian mammals. Proc. Biol. Sci. 1 October 2024; 291 (2033): 20241412. https://doi.org/10.1098/rspb.2024.1412"),
  plotOutput("plot", width="600px", height="500px")
)

server <- function(input, output, session) {
  output$plot <- renderPlot({
    gestation %>% filter(order == input$x) %>%
    ggplot(aes(x = .data[[input$y]])) +
      geom_density(fill = "blue",color="black", alpha = 0.75) +
      labs(x = input$y, title = paste("Density of", input$y, "for order", input$x)) +
      theme_minimal()
  })
}
...
ui <- 
  ...
  dashboardHeader(title = "Sex Composition by Population"),
