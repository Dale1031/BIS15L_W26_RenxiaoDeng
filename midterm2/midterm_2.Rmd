...
```{r}
register_stadiamaps("bb41e12a-8a3b-43de-a3ba-fc1a004b9543", write = FALSE)
```
...
NAs are represented by "unknown" in categorical data
```{r}
polar_bears_NAs <- polar_bears %>%
  replace_with_na_all(condition = ~.x %in%  "unknown")
polar_bears_NAs %>%
  miss_var_summary()
```
...
```{r}
polar_bears<-polar_bears_NAs %>%
  separate(capture_date,into=c("year","month","date"),sep="-") %>%
...
table %>%
  ...
  labs(title="Bears Captured By Year for Populations",
       x="Year",
       y="Bears")
```
...
```{r}
polar_bears_NAs %>%
  summarise(mean_urea_nitrogen = mean(urea_nitrogen),
...
polar_bears %>%
  ...
  scale_x_discrete()+
  theme(axis.text.x = element_text(angle=60, hjust=1))+
  labs(title="Fasting Bears By Year for Populations",
       x="Year",
       y="Bears")
...
ggmap(map1) +
  ...
  labs(x="Longitude",y="Latitude",title="Bear Records")
```
...
ui <- 
  ...
  dashboardHeader(title = "Un_create_ratio"),
 
  dashboardSidebar(
    selectInput("x",
              "Select Variables",
              choices = c("sex","mating","age_class","denning",
                          "fasting"))
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    polar_bears %>%
    ggplot(aes(y = un_creat_ratio,x=.data[[input$x]],fill=popn)) +
      geom_boxplot()
  })
}
