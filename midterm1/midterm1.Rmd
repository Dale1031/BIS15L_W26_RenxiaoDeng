...
Link to repository:
https://github.com/Dale1031/BIS15L_W26_RenxiaoDeng.git
\
\
\
\
...
anolis %>%
  ggplot(aes(x=island)) +
  geom_bar()+
  labs(title = "Sampling Across Islands", x = "islands", y = "samples count")
...
\
\
Sampling is not equal across the habitat as well.
\
\
\
```{r}
anolis %>%
  ...
  geom_bar()+
  labs(title = "Sampling Across Habitat", x = "Habitat", y = "samples count")
```
...
\
\
Twig has minimum range while Trunk_ground have the highest range of hindlimb length across habitat
\
```{r}
anolis %>%
  ggplot(aes(x=habitat,y=hindlimb_length_mm)) +
  geom_boxplot()+
  labs(title = "Hindlimb Length Across Habitat", x = "Habitat", y = "Hindlimb length")
```
...
\
The range of the data are more close to each other compare to the plot in problem 10.
\
```{r}
anolis %>%
  ggplot(aes(x=habitat,y=ratio_of_hindlimb_to_body)) +
  geom_boxplot()+
  labs(title = "Hindlimb Ratio To Body Across Habitat", x = "Habitat", y = "Hindlimb length ratio")
```
\
\
\
...
\
Generally the longer the tail length the longer the body length.
Lizards in Grass-Bush habitat have less body length(aka smaller) given the same tail length compare to other habitat, suggesting some factor that limit their body length.
\
```{r}
anolis %>%
  ggplot(aes(x=tail_length_mm,y=body_length_mm,colour = habitat)) +
  geom_point()+
  geom_smooth(method = lm, na.rm = T) +
  labs(title = "Tail length vs body length", x = "Tail Length", y = "Body Length")
```
\
\
\
...
anolis %>%
  group_by(habitat) %>%
  summarize(toepad_mean = mean(toepad_lamellae_count))
```
\
\
\
...
\
Yes, the difference is consistent across all the islands except Puerto Rico, the other three islands both have a mean significantly higher than the toepad mean of other species living in that island. 
\
```{r}
anolis %>% 
  ...
  geom_boxplot() +
  labs(title = "Toepad count in Trunk-crown lizards across islands", x = "Island", y = "Toepad Count")
```
