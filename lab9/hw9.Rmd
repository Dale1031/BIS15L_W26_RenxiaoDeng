...
```{r}
beaches <- read_csv("data/sydneybeaches.csv") %>% clean_names()
summary(beaches)
beaches
```

2. Are these data "tidy" per the definitions of the tidyverse? How do you know? Are they in wide or long format?
Yes, follow all the rules, in long format
```{r}
...
sydneybeaches_long %>%
  group_by(year,site) %>%
  summarize("Mean_ml" = mean(enterococci_cfu_100ml))
```

8. Make the output from question 7 easier to read by pivoting it to wide format.
```{r}
annul_pollution_wide <- sydneybeaches_long %>%
  group_by(year,site) %>%
  summarize("Mean_ml" = mean(enterococci_cfu_100ml)) %>%
  pivot_wider(names_from = "year",
              values_from = "Mean_ml")
annul_pollution_wide
```
...
annul_pollution_wide %>%
  ...
  pivot_longer(-site,
               names_to = "year",
              values_to = "enterococci_cfu_100ml") %>%
  ggplot(aes(x=year,y=enterococci_cfu_100ml))+geom_col()
