...
```{r}
p+geom_point(aes(color=thermoregulation),size=2.25)
```

We can also map shapes to another categorical variable.
```{r}
p+geom_point(aes(shape=thermoregulation,color=thermoregulation),size=2.25)
```

What about just filling in with a color preference or size?
```{r}
p+geom_point(size=3,color="black",alpha=0.8,shape=24,fill="skyblue")
```
```{r}
colors()
```
